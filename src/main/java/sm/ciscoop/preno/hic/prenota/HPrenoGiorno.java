...
/*-
 * Title:
 * Description:  Single per PrenoGiorno
 ...
 *
		...
		<servlet>
			<servlet-name>HPrenoGiorno</servlet-name>
				<servlet-class>sm.ciscoop.preno.hic.prenota.HPrenoGiorno</servlet-class>
			</servlet>
			<servlet-mapping>
				<servlet-name>HPrenoGiorno</servlet-name>
				<url-pattern>/servlet/sm.ciscoop.preno.hic.prenota.HPrenoGiorno</url-pattern>
			</servlet-mapping>
		</servlet>	
		...
		if @idnodo IS NULL
		  ...
		  END
		print @idnodo
		INSERT INTO zTrans ([CdPDC],[DsPDC],[IdNodo],[IdPadre],[Flag],[ultagg],[utente]) VALUES ('HPrenoGiorno' ,'HPrenoGiorno',@idnodo,0,0,GETDATE(),'appl')
		INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (1,  'HPrenoGiorno',' ','AGMIRVC',getdate(),'appl')
		--INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (xxx,  'HPrenoGiorno',' ','AGMIRVC',getdate(),'appl')
		
		// Voce di Menu.xml
		<Voce funzione="HPrenoGiorno" href="/prenota/HPrenoGiorno?MTipo=I&amp;MPasso=1">Single HPrenoGiorno</Voce>

 *
 */

@WebServlet(value=HPrenoGiorno.SERVLET_URL, name = HPrenoGiorno.SERVLET_NAME) 
public class HPrenoGiorno extends AppSingleMask<PrenoGiorno> {
	
	private static final long  serialVersionUID = 1L;
	
	public static final String SERVLET_NAME     = "HPrenoGiorno";
	public static final String SERVLET_URL      = "/prenota/" + HPrenoGiorno.SERVLET_NAME;
	...
	protected boolean afterExecIns(Passo nPasso, OpRes opRes) throws Exception {
		...
		if ( nPasso == Passo.ESECUZIONE && opRes == OpRes.OK) {
    	  ...
    	  */
				...
				setRedirect(getServletFullPath() + "?MTipo=V&MPasso=2"+"&IdPrenotazione="+getPDC().getIdPrenotazione());
