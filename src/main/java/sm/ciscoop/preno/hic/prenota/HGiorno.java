 ...
 *
	...
	if @idnodo IS NULL
	  ...
	  END
	print @idnodo
	INSERT INTO zTrans ([CdPDC],[DsPDC],[IdNodo],[IdPadre],[Flag],[ultagg],[utente]) VALUES ('HGiorno' ,'HGiorno',@idnodo,0,0,GETDATE(),'appl')
	INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (1, 'HGiorno',' ','LAGMIRVC',getdate(),'appl')
	--INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (xxx, 'HGiorno',' ','LAGMIRVC',getdate(),'appl')


	// Voce di Menu.xml
	<Voce funzione="HGiorno" href="/prenota/HGiorno">Single HGiorno</Voce>

 *
 */

@WebServlet(value=HGiorno.SERVLET_URL, name = HGiorno.SERVLET_NAME) 
public class HGiorno extends AppSingleMask<Giorno> {

	
	private static final long  serialVersionUID = 1L;

	public static final String SERVLET_NAME     = "HGiorno";
	public static final String SERVLET_URL      = "/prenota/" + HGiorno.SERVLET_NAME;
	...
	public boolean afterExecIns(Passo nPasso, OpRes opRes) throws Exception {
		...
		if ( nPasso == Passo.ESECUZIONE && opRes == opRes.OK) {
    	  ...
    	  */
			...
			setRedirect(getServletFullPath() + "?MTipo=V&MPasso=2"+"&IdGiorno="+getPDC().getIdGiorno());
