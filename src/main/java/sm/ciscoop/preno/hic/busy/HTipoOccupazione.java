...
/*-
 * Title:
 * Description:  Single per TipoOccupazione
 ...
 *
		...
		<servlet>
			<servlet-name>HTipoOccupazione</servlet-name>
				<servlet-class>sm.ciscoop.preno.hic.busy.HTipoOccupazione</servlet-class>
			</servlet>
			<servlet-mapping>
				<servlet-name>HTipoOccupazione</servlet-name>
				<url-pattern>/servlet/sm.ciscoop.preno.hic.busy.HTipoOccupazione</url-pattern>
			</servlet-mapping>
		</servlet>	
		...
		if @idnodo IS NULL
		  ...
		  END
		print @idnodo
		INSERT INTO zTrans ([CdPDC],[DsPDC],[IdNodo],[IdPadre],[Flag],[ultagg],[utente]) VALUES ('HTipoOccupazione' ,'HTipoOccupazione',@idnodo,0,0,GETDATE(),'appl')
		INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (1,  'HTipoOccupazione',' ','AGMIRVC',getdate(),'appl')
		--INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (xxx,  'HTipoOccupazione',' ','AGMIRVC',getdate(),'appl')
		
		// Voce di Menu.xml
		<Voce funzione="HTipoOccupazione" href="/busy/HTipoOccupazione?MTipo=I&amp;MPasso=1">Single HTipoOccupazione</Voce>

 *
 */

@WebServlet(value=HTipoOccupazione.SERVLET_URL, name = HTipoOccupazione.SERVLET_NAME) 
public class HTipoOccupazione extends AppSingleMask<TipoOccupazione> {
	
	private static final long  serialVersionUID = 1L;
	
	public static final String SERVLET_NAME     = "HTipoOccupazione";
	public static final String SERVLET_URL      = "/busy/" + HTipoOccupazione.SERVLET_NAME;
	...
	protected boolean afterExecIns(Passo nPasso, OpRes opRes) throws Exception {
		...
		if ( nPasso == Passo.ESECUZIONE && opRes == OpRes.OK) {
    	  ...
    	  */
				...
				setRedirect(getServletFullPath() + "?MTipo=V&MPasso=2"+"&TpOccup="+getPDC().getTpOccup());
