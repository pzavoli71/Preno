...
/*-
 * Title:
 * Description:  Single per Soggetto
 ...
 *
		...
		<servlet>
			<servlet-name>HSoggetto</servlet-name>
				<servlet-class>sm.ciscoop.preno.hic.soggetti.HSoggetto</servlet-class>
			</servlet>
			<servlet-mapping>
				<servlet-name>HSoggetto</servlet-name>
				<url-pattern>/servlet/sm.ciscoop.preno.hic.soggetti.HSoggetto</url-pattern>
			</servlet-mapping>
		</servlet>	
		...
		if @idnodo IS NULL
		  ...
		  END
		print @idnodo
		INSERT INTO zTrans ([CdPDC],[DsPDC],[IdNodo],[IdPadre],[Flag],[ultagg],[utente]) VALUES ('HSoggetto' ,'HSoggetto',@idnodo,0,0,GETDATE(),'appl')
		INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (1,  'HSoggetto',' ','AGMIRVC',getdate(),'appl')
		--INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (xxx,  'HSoggetto',' ','AGMIRVC',getdate(),'appl')
		
		// Voce di Menu.xml
		<Voce funzione="HSoggetto" href="/soggetti/HSoggetto?MTipo=I&amp;MPasso=1">Single HSoggetto</Voce>

 *
 */

@WebServlet(value=HSoggetto.SERVLET_URL, name = HSoggetto.SERVLET_NAME) 
public class HSoggetto extends AppSingleMask<Soggetto> {
	
	private static final long  serialVersionUID = 1L;
	
	public static final String SERVLET_NAME     = "HSoggetto";
	public static final String SERVLET_URL      = "/soggetti/" + HSoggetto.SERVLET_NAME;
	...
	c_IdSoggetto = attrs.addIntAttr("IdSoggetto");
		c_NomeSoggetto = attrs.addCharAttr("NomeSoggetto");
		c_EmailSogg = attrs.addCharAttr("EmailSogg");
...
//Soggetto.CSZrel_PrenoGiornoandat,10,
//Soggetto.CSZrel_PrenoGiornoritor,10,
	...
	protected boolean afterExecIns(Passo nPasso, OpRes opRes) throws Exception {
		...
		if ( nPasso == Passo.ESECUZIONE && opRes == OpRes.OK) {
    	  ...
    	  */
				...
				setRedirect(getServletFullPath() + "?MTipo=V&MPasso=2"+"&IdSoggetto="+getPDC().getIdSoggetto());
