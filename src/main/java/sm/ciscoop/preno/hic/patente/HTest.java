 ...
 *
	...
	if @idnodo IS NULL
	  ...
	  END
	print @idnodo
	INSERT INTO zTrans ([CdPDC],[DsPDC],[IdNodo],[IdPadre],[Flag],[ultagg],[utente]) VALUES ('HTest' ,'HTest',@idnodo,0,0,GETDATE(),'appl')
	INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (1, 'HTest',' ','LAGMIRVC',getdate(),'appl')
	--INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (xxx, 'HTest',' ','LAGMIRVC',getdate(),'appl')


	// Voce di Menu.xml
	<Voce funzione="HTest" href="/patente/HTest">Single HTest</Voce>

 *
 */

@WebServlet(value=HTest.SERVLET_URL, name = HTest.SERVLET_NAME) 
public class HTest extends AppSingleMask<Test> {

	
	private static final long  serialVersionUID = 1L;

	public static final String SERVLET_NAME     = "HTest";
	public static final String SERVLET_URL      = "/patente/" + HTest.SERVLET_NAME;
	...
	public boolean afterExecIns(Passo nPasso, OpRes opRes) throws Exception {
		...
		if ( nPasso == Passo.ESECUZIONE && opRes == opRes.OK) {
    	  ...
    	  */
			...
			setRedirect(getServletFullPath() + "?MTipo=V&MPasso=2"+"&IdTest="+getPDC().getIdTest());
