...
/*-
 * Title:
 * Description:  Single per RispQuiz
 ...
 *
	...
	if @idnodo IS NULL
	  ...
	  END
	print @idnodo
	INSERT INTO zTrans ([CdPDC],[DsPDC],[IdNodo],[IdPadre],[Flag],[ultagg],[utente]) VALUES ('HRispQuiz' ,'HRispQuiz',@idnodo,0,0,GETDATE(),'appl')
	INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (1, 'HRispQuiz',' ','LAGMIRVC',getdate(),'appl')
	--INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (xxx, 'HRispQuiz',' ','LAGMIRVC',getdate(),'appl')


	// Voce di Menu.xml
	<Voce funzione="HRispQuiz" href="/patente/HRispQuiz">Single HRispQuiz</Voce>

 *
 */

@WebServlet(value=HRispQuiz.SERVLET_URL, name = HRispQuiz.SERVLET_NAME) 
public class HRispQuiz extends AppSingleMask<RispQuiz> {

	
	private static final long  serialVersionUID = 1L;

	public static final String SERVLET_NAME     = "HRispQuiz";
	public static final String SERVLET_URL      = "/patente/" + HRispQuiz.SERVLET_NAME;
...
*/
	...
	public void loadCombo() throws Exception {
		...
		if ( isSoloDatiCombo()) {
			...
			if (camporicerca.equalsIgnoreCase("IdDomandaTest")) {
				String ls = "select top 100 IdDomandaTest, IdCapitolo as nome FROM "+DomandaQuiz.CSZ_DBTable;
				ls += " WHERE 1 = 1";
				if ( desc != null && desc.length() >= 1) {
					desc = desc.replaceAll("'", "''");
					ls += " AND IdCapitolo like '"+desc.trim()+"%'";
				} else if ( cd != null && cd.length() > 0) {
					ls += " AND IdDomandaTest = "+cd.trim();
				} else
					ls += " AND 1 = 0";
				appendToResponse(new AnySelect(ls, "Elementi"));
			}
				...
				String lsDomandaQuiz = "select IdDomandaTest, IdCapitolo FROM "+DomandaQuiz.CSZ_DBTable;
				lsDomandaQuiz += " WHERE 1 = 0 "; // Ricordarsi di abilitare il combo togliendo questo filtro
				appendToResponse(new AnySelect(lsDomandaQuiz, "DomandaQuiz"));
	...
	public boolean afterExecIns(Passo nPasso, OpRes opRes) throws Exception {
		...
		if ( nPasso == Passo.ESECUZIONE && opRes == opRes.OK) {
    	  ...
    	  */
			...
			setRedirect(getServletFullPath() + "?MTipo=V&MPasso=2"+"&IdRispTest="+getPDC().getIdRispTest());
