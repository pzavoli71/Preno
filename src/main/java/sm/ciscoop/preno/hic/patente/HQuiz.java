 ...
 *
	...
	if @idnodo IS NULL
	  ...
	  END
	print @idnodo
	INSERT INTO zTrans ([CdPDC],[DsPDC],[IdNodo],[IdPadre],[Flag],[ultagg],[utente]) VALUES ('HQuiz' ,'HQuiz',@idnodo,0,0,GETDATE(),'appl')
	INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (1, 'HQuiz',' ','LAGMIRVC',getdate(),'appl')
	--INSERT INTO zPermessi([CdGruppo],[CdPDC], [Descrizione], [Permesso], [ultagg], [utente]) VALUES (xxx, 'HQuiz',' ','LAGMIRVC',getdate(),'appl')


	// Voce di Menu.xml
	<Voce funzione="HQuiz" href="/patente/HQuiz">Single HQuiz</Voce>

 *
 */

@WebServlet(value=HQuiz.SERVLET_URL, name = HQuiz.SERVLET_NAME) 
public class HQuiz extends AppSingleMask<Quiz> {

	
	private static final long  serialVersionUID = 1L;

	public static final String SERVLET_NAME     = "HQuiz";
	public static final String SERVLET_URL      = "/patente/" + HQuiz.SERVLET_NAME;
	...
	public void loadCombo() throws Exception {
		...
		if ( isSoloDatiCombo()) {
			...
			// Todo Aggiungere qui i campi filtro dinamici
				...
				lszUtente += " WHERE 1 =1 "; // Ricordarsi di abilitare il combo togliendo questo filtro
	...
	public boolean afterExecIns(Passo nPasso, OpRes opRes) throws Exception {
		...
		if ( nPasso == Passo.ESECUZIONE && opRes == opRes.OK) {
    	  ...
    	  */
			...
			setRedirect(getServletFullPath() + "?MTipo=V&MPasso=2"+"&IdQuiz="+getPDC().getIdQuiz());
