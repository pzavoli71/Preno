<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0">
    <xsl:import href="${AppThemes}/atlanta-1.1.0-SNAPSHOT/xsl/theme.xsl"/>
	<xsl:import href="${AppThemes}/atlanta-1.1.0-SNAPSHOT/xsl/forms2.xsl" />
    <xsl:import href="${AppXSL}/app.xsl"/>
    
	<xsl:output method='html' indent='yes'/>
	
	<xsl:template match="/">
		<xsl:call-template name="baseForm" >
			<xsl:with-param name="tooltips">true</xsl:with-param>
			<xsl:with-param name="noform">true</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	
	<xsl:template name="CssSpec" >
		<style>
		.cis-field-container label {
			width:150px;
		}
		</style>
	</xsl:template>
	
	<xsl:variable name="TitoloPagina"><xsl:value-of select="$tipo_transaz"/> Ruolo</xsl:variable>
	
	<xsl:template  name="def_javascript">
		function getTitolo(){
		  return '  '  + '<xsl:value-of select="$tipo_transaz"/>' + ' <xsl:value-of select="$TitoloPagina"/>' + '   ';
		}
	</xsl:template>
	
	<xsl:template match="DOCUMENTO">
		<xsl:apply-templates select="zRuolo/IdRuolo" mode="CampoIntero">
			<xsl:with-param name="size">10</xsl:with-param>
			<xsl:with-param name="chiave">true</xsl:with-param>
			<xsl:with-param name="caption">IdRuolo</xsl:with-param>
		</xsl:apply-templates>
		<xsl:apply-templates select="zRuolo/NomeRuolo" mode="CampoTesto">
			<xsl:with-param name="size">40</xsl:with-param>
			<xsl:with-param name="caption">NomeRuolo</xsl:with-param>
		</xsl:apply-templates>
    </xsl:template>
</xsl:stylesheet>
