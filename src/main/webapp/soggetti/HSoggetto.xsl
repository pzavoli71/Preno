...
<xsl:variable name="TitoloPagina"><xsl:value-of select="$tipo_transaz"/>Soggetto</xsl:variable>
...
<xsl:template match="DOCUMENTO" >
    ...
    <xsl:apply-templates select = "Soggetto"/>
    
    <xsl:call-template name = "Errori"/>
</xsl:template>

 
<xsl:template match="Soggetto">

	<xsl:apply-templates select="IdSoggetto" mode="CampoIntero">
		<xsl:with-param name="size">10</xsl:with-param>
		<xsl:with-param name="chiave">true</xsl:with-param>
		<xsl:with-param name="readonly">true</xsl:with-param>
		<xsl:with-param name="caption">Id Soggetto</xsl:with-param>
	</xsl:apply-templates>

	<xsl:apply-templates select="NomeSoggetto" mode="CampoTesto">
		<xsl:with-param name="size">30</xsl:with-param>		
		<xsl:with-param name="caption">Nome Soggetto</xsl:with-param>
	</xsl:apply-templates>

	<xsl:apply-templates select="EmailSogg" mode="CampoTesto">
		<xsl:with-param name="size">30</xsl:with-param>		
		<xsl:with-param name="caption">Email</xsl:with-param>
	</xsl:apply-templates>

	<xsl:apply-templates select="bRagazzo" mode="CampoCheckbox">
		<xsl:with-param name="size">10</xsl:with-param>		
		<xsl:with-param name="caption">E' un Ragazzo?</xsl:with-param>
	</xsl:apply-templates>

	<xsl:apply-templates select="CodISS" mode="CampoIntero">
		<xsl:with-param name="size">10</xsl:with-param>
		
		<xsl:with-param name="caption">CodISS</xsl:with-param>
	</xsl:apply-templates>
