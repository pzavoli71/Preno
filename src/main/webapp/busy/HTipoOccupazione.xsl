...
<xsl:variable name="TitoloPagina"><xsl:value-of select="$tipo_transaz"/>TipoOccupazione</xsl:variable>
...
<xsl:template match="DOCUMENTO" >
    ...
    <xsl:apply-templates select = "TipoOccupazione"/>
    
    <xsl:call-template name = "Errori"/>
</xsl:template>

 
<xsl:template match="TipoOccupazione">

	<xsl:apply-templates select="TpOccup" mode="CampoIntero">
		<xsl:with-param name="size">10</xsl:with-param>
		<xsl:with-param name="chiave">true</xsl:with-param>
		<xsl:with-param name="caption">TpOccup</xsl:with-param>
	</xsl:apply-templates>

	<xsl:apply-templates select="DsOccup" mode="CampoTesto">
		<xsl:with-param name="size">10</xsl:with-param>
		
		<xsl:with-param name="caption">DsOccup</xsl:with-param>
	</xsl:apply-templates>
