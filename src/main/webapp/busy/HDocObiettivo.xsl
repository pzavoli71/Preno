...
<xsl:variable name="TitoloPagina"><xsl:value-of select="$tipo_transaz"/>DocObiettivo</xsl:variable>
...
<xsl:template match="DOCUMENTO" >
    ...
    <xsl:apply-templates select = "DocObiettivo"/>
    
    <xsl:call-template name = "Errori"/>
</xsl:template>

 
<xsl:template match="DocObiettivo">

	<xsl:apply-templates select="IdObiettivo" mode="CampoEnumeratoX">
		...
		<xsl:with-param name="enumerato" select="DOCUMENTO/LstObiettivo"></xsl:with-param>
		<xsl:with-param name="servlet">/preno/busy/HLstDocObiettivo</xsl:with-param>
		<xsl:with-param name="campocodice">idobiettivo</xsl:with-param> <!--Nome del campo codice della simplelist dell'enumerato -->
	...
	<xsl:apply-templates select="IdDocObiettivo" mode="CampoIntero">
		<xsl:with-param name="size">10</xsl:with-param>
		<xsl:with-param name="chiave">true</xsl:with-param><xsl:with-param name="serial">true</xsl:with-param>
		<xsl:with-param name="caption">IdDocObiettivo</xsl:with-param>
	</xsl:apply-templates>

	<xsl:apply-templates select="DtDoc" mode="// Tipo dato DATETIMEYM di DtDoc  Non previsto ">
		<xsl:with-param name="size">10</xsl:with-param>
		
		<xsl:with-param name="caption">DtDoc</xsl:with-param>
	</xsl:apply-templates>

	<xsl:apply-templates select="PathDoc" mode="CampoTesto">
		<xsl:with-param name="size">10</xsl:with-param>
		
		<xsl:with-param name="caption">PathDoc</xsl:with-param>
	</xsl:apply-templates>

	<xsl:apply-templates select="NotaDoc" mode="CampoTesto">
		<xsl:with-param name="size">10</xsl:with-param>
		
		<xsl:with-param name="caption">NotaDoc</xsl:with-param>
	</xsl:apply-templates>
