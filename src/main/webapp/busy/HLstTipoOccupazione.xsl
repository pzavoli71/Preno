...
<xsl:variable name="TitoloPagina">TipoOccupazione</xsl:variable>
...
function getAutoHref(){
	var href="/preno/busy/HLstTipoOccupazione?MTipo=L&amp;MPasso=1";
	return href;
}
...
function caricaRelazione(obj) { //atag,rigapos, nomepdc, nomerelazione) {
	...
	*/
	...
	AppGlob.reloadRelazione("/preno/busy/HLstTipoOccupazione",nomepdc,nomerelazione,dati,$odivRelaz, function(dati, data) { });
	...
	<form method="GET" id="Parametri" name="Parametri">
		...
		<legend>Imposta il filtro</legend>
			<div class="filtrosinistra">
				...
				<xsl:apply-templates select="/DOCUMENTO/TipoOccupazione/TpOccup" mode="nodoattributo" />
...
<xsl:template name="IntestazioneLista">
		...
		<a href="javascript:void(0)" onclick="document.location.reload()" class="refresh_btn cis-button"><i class="fa fa-refresh"/></a>
			<xsl:call-template name="ButtonHref">
				<xsl:with-param name="href">/preno/busy/HTipoOccupazione?MTipo=I&amp;MPasso=1</xsl:with-param>
				<xsl:with-param name="title">Aggiungi un TipoOccupazione</xsl:with-param>
				...
				<xsl:with-param name="caption">Gestione TipoOccupazione</xsl:with-param>
				<!--xsl:with-param name="text">TipoOccupazione</xsl:with-param-->
			</xsl:call-template>
		</div>
</xsl:template>
...
<!--    Righe tabella                             -->
<!-- ============================================ -->
<xsl:template match="TipoOccupazione" mode="lista" >
<xsl:param name="rigapos"/>

   <tr id='RigaTipoOccupazione_{$rigapos}' chiave="{TpOccup}">
    <xsl:choose>
      ...
      <xsl:otherwise><xsl:attribute name="class">rigaPari</xsl:attribute></xsl:otherwise>
		...
		<td style="text-align:left">
			<xsl:call-template name="BottoniRiga">
				<xsl:with-param name="nomepdc">TipoOccupazione</xsl:with-param>
				<xsl:with-param name="pos"><xsl:value-of select="$rigapos"/></xsl:with-param>
				<xsl:with-param name="ramochiuso">true</xsl:with-param>
			</xsl:call-template>
			<xsl:call-template name = "ButtonHref" >
				<xsl:with-param name="href">/preno/busy/HTipoOccupazione?MTipo=V&amp;MPasso=2&amp;TpOccup=<xsl:value-of select="TpOccup"/></xsl:with-param>
				<xsl:with-param name="title">Modifica un TipoOccupazione</xsl:with-param>
				<xsl:with-param name="fa-class">fa-edit</xsl:with-param>
				<xsl:with-param name="onunload">document.location.reload()</xsl:with-param>
				<xsl:with-param name="caption">Gestione TipoOccupazione</xsl:with-param>
				<!--xsl:with-param name="text">Gestione TipoOccupazione</xsl:with-param-->
			</xsl:call-template>

		</td>

		<td><xsl:value-of select="TpOccup"/></td>

		<td><xsl:value-of select="DsOccup"/></td>
		...
		<!--td align="right" class="bottoni_comando">
 		    <xsl:call-template name="linkComando">
		    	<xsl:with-param name="href">/preno/busy/HLstTipoOccupazione</xsl:with-param>
		    	<xsl:with-param name="nomecomando">ElaboraMeseSmac</xsl:with-param>
		    	<xsl:with-param name="chiave"><xsl:value-of select='TpOccup'/></xsl:with-param>
			...
			<a href="javascript:void(0)"  style="margin-left:1px; margin-right:5px;font-size:1.5em" onclick="AppGlob.eseguiComando('/preno/busy/HLstTipoOccupazione', 'CreaPiattaforma', '{TpOccup}', {{MTipo:'L',MPasso:'2'}}, richiestaComando, comandoTerminato)" title="Elabora il comando">
...
<!--    Relazioni tra i pdc                       -->
<!-- ============================================ -->
<xsl:template name="RelazioniTipoOccupazione">
	<xsl:param name="rigapos"><xsl:value-of select="position()"/></xsl:param>
	<tr id="RigaRelTipoOccupazione_{$rigapos}">
		<xsl:choose>
		<xsl:when test="(position() mod 2) = 1"><xsl:attribute name="class">rigaDispari</xsl:attribute></xsl:when>
			<xsl:otherwise><xsl:attribute name="class">rigaPari</xsl:attribute></xsl:otherwise>
		</xsl:choose>
		<td colspan="100" class="closed tdRelazione" >
			...
			<xsl:when test="count(ListaTipoOccupazione_Obiettivo/*[@progr or @result]) &gt; 0"><xsl:attribute name="class">open</xsl:attribute></xsl:when>
		</xsl:choose-->


    <div style="margin-left:20px;" id="divRel_TipoOccupazione_Obiettivo_{position()}" class="divRelazione" chiave="{TpOccup}" nomepdc="sm.ciscoop.preno.pdc.busy.TipoOccupazione" nomerelaz="TipoOccupazione_Obiettivo">
		<div class="titolorelaz"><a class="refresh_btn cis-button" href="javascript:void(0)" onclick="caricaRelazione(this)">
		<i class="fa fa-refresh"/>
		</a>
		<xsl:call-template name = "ButtonHref" >
			<xsl:with-param name="href">/preno/busy/HObiettivo?MTipo=I&amp;MPasso=1&amp;TpOccup=<xsl:value-of select="TpOccup"/></xsl:with-param>
		...
		<xsl:apply-templates select="ListaTipoOccupazione_Obiettivo" mode="tmlRel"><xsl:with-param name="pos"><xsl:value-of select="position()"/></xsl:with-param></xsl:apply-templates>
		</div>
...
<xsl:template match="ListaTipoOccupazione_Obiettivo" mode="tmlRel">
<xsl:param name="pos"/>
		...
		<table border="0" cellpadding="2" cellspacing="0" class="tabLista" id="tabListaTipoOccupazione_Obiettivo_{$pos}" nomepdc="TipoOccupazione">
			<xsl:call-template name="IntestaTabellaObiettivo"/>
		...
		<!-- Scommentare per fare una transazione T con uesta tabella <script language="javascript">
				$('#tabListaTipoOccupazione_Obiettivo_<xsl:value-of select="$pos"/>').cisTTrans();
		</script-->		
		</div>
		</xsl:if>
</xsl:template>

<xsl:template name="IntestaTabellaObiettivo">
	<tr>
		<th style="min-width:180px"></th>
...
<!--    Righe tabella                             -->
<!-- ============================================ -->
<xsl:template match="Obiettivo" mode="lista" >

	<tr id='RigaObiettivo_{position()}' chiave='{IdObiettivo}'>
		<xsl:choose>
			<xsl:when test="(position() mod 2) = 1"><xsl:attribute name="class">rigaDispari</xsl:attribute></xsl:when>
			<xsl:otherwise><xsl:attribute name="class">rigaPari</xsl:attribute></xsl:otherwise>
		</xsl:choose>
		<td style="text-align:left">
			...
			<xsl:call-template name="ButtonHref">
				...
				<xsl:with-param name="fa-class">fa-edit</xsl:with-param>
				<xsl:with-param name="onunload">caricaRelazione(this.atag)</xsl:with-param><!--$("#divRel_TipoOccupazione_Obiettivo_<xsl:value-of select='TpOccup'/> > div > a.refresh_btn").click()-->
				<xsl:with-param name="caption">Gestione Obiettivo</xsl:with-param>
			</xsl:call-template>
		</td>
...
<!-- ============================================ -->
	...
	<xsl:apply-templates select="/DOCUMENTO/TipoOccupazione/TpOccup" mode="CampoIntero">
		<xsl:with-param name="size">10</xsl:with-param>
		<xsl:with-param name="chiave">true</xsl:with-param>
		<xsl:with-param name="caption">TpOccup</xsl:with-param>
	</xsl:apply-templates>
...
<!--xsl:call-template name="crea_iframe" >
   <xsl:with-param name="id" >fmTipoOccupazione</xsl:with-param>
   <xsl:with-param name="name" >fmTipoOccupazione</xsl:with-param>
   <xsl:with-param name="titolo">TipoOccupazione</xsl:with-param>
