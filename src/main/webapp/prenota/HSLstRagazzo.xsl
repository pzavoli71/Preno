...
<xsl:variable name="TitoloPagina">HSLstRagazzo</xsl:variable>
...
function getAutoHref(){
	var href="/preno/prenota/HSLstRagazzo?MTipo=L&amp;MPasso=1";
	return href;
}
...
function caricaRelazione(obj) { //atag,rigapos, nomepdc, nomerelazione) {
	...
	*/
	...
	AppGlob.reloadRelazione("/preno/prenota/HSLstRagazzo",nomepdc,nomerelazione,dati,$odivRelaz, function(dati, data) { });
  ...
  <form method="GET" id="Parametri" name="Parametri">
		...
		<legend>Imposta il filtro</legend>
			<div class="filtrosinistra">
				...
				<xsl:apply-templates select="/DOCUMENTO/Ragazzo/IdSoggRagazzo" mode="nodoattributo" />
...
<xsl:template name="IntestazioneLista">
		...
		<a href="javascript:void(0)" onclick="document.location.reload()" class="refresh_btn cis-button"><i class="fa fa-refresh"/></a>
			<xsl:call-template name = "ButtonHref" >
				<xsl:with-param name="href">/preno/prenota/HRagazzo?MTipo=I&amp;MPasso=1</xsl:with-param>
				...
				<!--xsl:with-param name="target">fmRagazzo</xsl:with-param-->
			...
			<!--span class="titolo1">Ragazzo</span-->
		</div>
</xsl:template>
...
<xsl:template name="IntestaTabella">
	<tr >
		...
		<th data-nomecol='NomeRagazzo'>NomeRagazzo</th>

		<th data-nomecol='EmailRagazzo'>EmailRagazzo</th>
...
<xsl:param name="rigapos"/>
   ...
   <tr id='RigaRagazzo_{$rigapos}' chiave="{idsoggragazzo}">
    <xsl:choose>
      ...
      <xsl:otherwise><xsl:attribute name="class">rigaPari</xsl:attribute></xsl:otherwise>
		...
		<td>
			<xsl:call-template name="BottoniRiga">
				<xsl:with-param name="nomepdc">Ragazzo</xsl:with-param>
				<xsl:with-param name="pos"><xsl:value-of select="$rigapos"/></xsl:with-param>
				<xsl:with-param name="ramochiuso">true</xsl:with-param>
			</xsl:call-template>
			<xsl:call-template name="ButtonHref">
				<xsl:with-param name="href">/preno/prenota/HRagazzo?MTipo=V&amp;MPasso=1&amp;IdSoggRagazzo=<xsl:value-of select="idsoggragazzo"/></xsl:with-param>
	            <xsl:with-param name="title">Modifica un Ragazzo</xsl:with-param>
		...
		<td><xsl:value-of select="idsoggragazzo"/></td>

		<td><xsl:value-of select="nomeragazzo"/></td>

		<td><xsl:value-of select="emailragazzo"/></td>
		...
		<!--td align="right" class="bottoni_comando">
 		    <xsl:call-template name="linkComando">
		    	<xsl:with-param name="href">/preno/prenota/HSLstRagazzo</xsl:with-param>
		    	<xsl:with-param name="nomecomando">ElaboraMeseSmac</xsl:with-param>
		    	<xsl:with-param name="chiave"><xsl:value-of select='IdSoggRagazzo'/></xsl:with-param>
			...
			<a href="javascript:void(0)"  style="margin-left:1px; margin-right:5px;font-size:1.5em" onclick="AppGlob.eseguiComando('/preno/prenota/HSLstRagazzo', 'CreaPiattaforma', '{idsoggragazzo}', {{MTipo:'L',MPasso:'2'}}, richiestaComando, comandoTerminato)" title="Elabora il comando">
	...
	<xsl:call-template name="RelazioniRagazzo">
...
<!--    Relazioni tra i pdc                       -->
<!-- ============================================ -->
<xsl:template name="RelazioniRagazzo">
<xsl:param name="rigapos"><xsl:value-of select="position()"/></xsl:param>
	<tr id="RigaRelRagazzo_{$rigapos}">
    <xsl:choose>
      <xsl:when test="(position() mod 2) = 1"><xsl:attribute name="class">rigaDispari</xsl:attribute></xsl:when>
      <xsl:otherwise><xsl:attribute name="class">rigaPari</xsl:attribute></xsl:otherwise>
    </xsl:choose>
    <td colspan="100" class="closed tdRelazione" >
		...
		<xsl:when test="'1' = '2'"></xsl:when>
			<xsl:when test="count(ListaRagazzo_PrenoRagazzo/*[@progr or @result]) &gt; 0"><xsl:attribute name="class">open</xsl:attribute></xsl:when>
		</xsl:choose-->


    <div style="margin-left:20px;" id="divRel_Ragazzo_PrenoRagazzo_{position()}" class="divRelazione" chiave="{idsoggragazzo}" nomepdc="sm.ciscoop.preno.pdc.prenota.Ragazzo" nomerelaz="Ragazzo_PrenoRagazzo">
		<div class="titolorelaz"><a class="refresh_btn cis-button" href="javascript:void(0)" onclick="caricaRelazione(this)">
		<i class="fa fa-refresh"/>
		</a>
		<xsl:call-template name = "ButtonHref" >
			<xsl:with-param name="href">/preno/prenota/HPrenoRagazzo?MTipo=I&amp;MPasso=1&amp;IdSoggRagazzo=<xsl:value-of select="idsoggragazzo"/></xsl:with-param>
		...
		<xsl:apply-templates select="Ragazzo_PrenoRagazzo" mode="tmlRel"><xsl:with-param name="pos"><xsl:value-of select="position()"/></xsl:with-param></xsl:apply-templates>
		</div>
...
<xsl:template match="ListaRagazzo_PrenoRagazzo" mode="tmlRel" >
<xsl:param name="pos"/>
		...
		<table border="0" cellpadding="2" cellspacing="0" class="tabLista" id="tabListaRagazzo_PrenoRagazzo_{$pos}" nomepdc="Ragazzo">
		...
		<!-- Scommentare per fare una transazione T con uesta tabella <script language="javascript">
				$('#tabListaRagazzo_PrenoRagazzo_<xsl:value-of select="$pos"/>').cisTTrans();
		</script-->
		</div>
		</xsl:if>
</xsl:template>
      ...
      <xsl:otherwise><xsl:attribute name="class">rigaPari</xsl:attribute></xsl:otherwise>
		...
		<td>
			...
			<xsl:call-template name = "ButtonHref" >
				...
				<xsl:with-param name="onunload">caricaRelazione(this.atag)</xsl:with-param><!--$("#divRel_Ragazzo_PrenoRagazzo_<xsl:value-of select='IdSoggRagazzo'/> > div > a.refresh_btn").click()-->
				<xsl:with-param name="caption">Gestione PrenoRagazzo</xsl:with-param>
			</xsl:call-template>

		</td>

		<td><xsl:value-of select="IdPrenotazione"/><xsl:value-of select="PrenoGiorno/nome"></xsl:value-of></td>

		<td><xsl:value-of select="IdSoggRagazzo"/><xsl:value-of select="Ragazzo/nome"></xsl:value-of></td>
...
<xsl:param name="btn_menu">false</xsl:param>
	...
	<a pos="{$pos}" href="javascript:void(0)" style="margin-left:1px; margin-right:5px" onclick="AppGlob.apriRigaRelazioni(this, 'sm.ciscoop.preno.pdc.prenota.{$nomepdc}')" class="btn_espandi">
...
<!--xsl:call-template name="crea_iframe" >
   <xsl:with-param name="id" >fmRagazzo</xsl:with-param>
   <xsl:with-param name="name" >fmRagazzo</xsl:with-param>
   <xsl:with-param name="titolo">Ragazzo</xsl:with-param>
